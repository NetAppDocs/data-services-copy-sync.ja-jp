...
---
= NetApp Copy and Sync のソースとターゲットの要件
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: コピーと同期を使用して SMB データをCloud Volumes ONTAPにコピーする場合、ソース システムのファイルとフォルダーの所有権は保持されません。この動作は、コピーと同期が Linux SMB クライアントを使用し、転送の認証に使用されるユーザーまたはサービス アカウントに所有権を割り当てるために発生します。アクセス制御リストは保持される場合がありますが、所有権と監査情報はソース システムとは異なる場合があります。これは想定される動作です。
...
顧客管理の KMS キーを追加するには、link:task-installing-gcp.html#permissions-required-for-the-service-account["正しい権限"]を使用するデータブローカーが必要です。また、キーはバケットと同じリージョンに存在する必要があります。
