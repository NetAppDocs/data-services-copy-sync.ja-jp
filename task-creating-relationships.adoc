...
** https://docs.netapp.com/us-en/storage-management-azure-netapp-files/task-quick-start.html["Azure NetApp Files の設定と検出"^]
...
** https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-adding-systems.html["既存のCloud Volumes ONTAPシステムの追加"^]
** https://docs.netapp.com/us-en/storage-management-ontap-onprem/task-discovering-ontap.html["ONTAPクラスターの検出"^]
...
+
スケジュール:: 今後の同期のために定期的なスケジュールを選択するか、同期スケジュールをオフにしてください。データ同期の頻度を最短1分間隔に設定できます。スケジュールは、1時間ごと、または1日ごとの頻度で設定することもできます。
+
--
...
コピーと同期機能は、ソースとターゲット間でデータの同期を開始します。同期にかかった時間、同期が中断したかどうか、コピー、スキャン、削除されたファイルの数などの同期統計情報が利用可能です。その後、link:task-managing-relationships.html["同期関係"]、link:task-managing-data-brokers.html["データブローカーを管理する"]、またはlink:task-managing-reports.html#creating-reports["パフォーマンスと構成を最適化するためのレポートを作成する"]を管理できます。
...
https://docs.netapp.com/us-en/data-services-data-classification/concept-classification.html["NetApp Data Classificationから同期関係を開始する方法を学びます"^]。
