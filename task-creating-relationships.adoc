...
コピーと同期機能は、ソースとターゲット間でデータの同期を開始します。同期にかかった時間、同期が中断したかどうか、コピー、スキャン、削除されたファイルの数などの同期統計情報が利用可能です。その後、link:task-managing-relationships.html["同期関係"]、link:task-managing-data-brokers.html["データブローカーを管理する"]、またはlink:task-managing-reports.html["パフォーマンスと構成を最適化するためのレポートを作成する"]を管理できます。
