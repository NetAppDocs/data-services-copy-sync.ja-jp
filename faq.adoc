...
NOTE: Copy Sync は SMB ACL (アクセス許可) をコピーしますが、ファイルまたはフォルダーの所有権はコピーしません。所有権属性は、SMB ACL コピー操作には含まれません。 SMB共有間でデータをコピーする際に所有権を保持する必要がある場合は、 `robocopy`セキュリティ情報を手動でコピーします。例えば、 `/copyall`フラグは ACL、所有者、監査データをコピーします。
