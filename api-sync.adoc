...
summary: Web UI を通じて利用できるNetApp のコピーおよび同期機能は、RESTful API を通じても利用できます。 
---
= NetAppコピーおよび同期 API
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
Web UI を通じて利用できるNetApp のコピーおよび同期機能は、RESTful API を通じても利用できます。



== 始めましょう

コピーおよび同期 API の使用を開始するには、ユーザー トークンとNetAppコンソール アカウント ID を取得する必要があります。  API 呼び出しを行うときは、トークンとアカウント ID を Authorization ヘッダーに追加する必要があります。

.手順
. NetAppコンソールからユーザー トークンを取得します。
+
[source, http]
----
...
.次の手順
NetAppコンソールからのユーザー トークンには有効期限があります。トークンを更新するには、手順 1 から API を再度呼び出す必要があります。
...
電話すると `list-s3-buckets`API では、結果はすぐには返されません。
